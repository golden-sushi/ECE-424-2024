...
def generate_rsa_keys(bits=2048):
    ...
    #e such that 1 < e < phi(n) and gcd(e, phi) = 1
    e = 65537 

    #d (the modular multiplicative inverse of e mod phi)
    d = pow(e, -1, phi)
...
def rsa_encrypt(public_key, message):
    e, n = public_key
    message_chunks = chunk_message(message, (n.bit_length() // 8) - 1)
    ciphertext_chunks = []
...
#Bob -> Alice: E(KE_A, NA || NB)
nb = random.randint(1, 1000000)  # Random nonce
bob_to_alice_message = f"{na}||{nb}"
...
#successful exchange and verification
def verify_exchange():
    #decrypt Bob -> Alice message
    decrypted_bob_to_alice = rsa_decrypt(alice_private_key, bob_to_alice_encrypted)
    decrypted_na, decrypted_nb = map(int, decrypted_bob_to_alice.split("||"))
    assert decrypted_na == na, "NA mismatch"
    ...
    print("Bob to Alice response message:", f"{na}||{nb}")
